...
Route::middleware('guest.app.lock')->group(function (): void {
    Route::get('/login', [AppLockController::class, 'showLogin'])->name('auth.login');
    Route::post('/login', [AppLockController::class, 'login'])->name('auth.login.submit');
    Route::post('/forgot-password', [AppLockController::class, 'forgotPassword'])->name('auth.forgot');
});

Route::post('/logout', [AppLockController::class, 'logout'])->name('auth.logout')->middleware('app.lock');

Route::middleware('app.lock')->group(function (): void {
